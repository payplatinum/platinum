...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Platinum.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Platinumd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePlatinumamount=<n> " + _("Keep N Platinum anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Platinum is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Platinum is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Platinum version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizePlatinumAmount = GetArg("-anonymizePlatinumamount", 0);
    if(nAnonymizePlatinumAmount > 999999) nAnonymizePlatinumAmount = 999999;
    if(nAnonymizePlatinumAmount < 2) nAnonymizePlatinumAmount = 2;
    ...
    LogPrintf("Anonymize Platinum Amount %d\n", nAnonymizePlatinumAmount);
