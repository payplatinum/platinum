...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePlatinumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePlatinumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePlatinumAmount = strAnonymizePlatinumAmount.remove(strAnonymizePlatinumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePlatinumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePlatinumAmount*COIN) nMaxToAnonymize = nAnonymizePlatinumAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePlatinumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePlatinumAmount));
        strAnonymizePlatinumAmount = strAnonymizePlatinumAmount.remove(strAnonymizePlatinumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePlatinumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
