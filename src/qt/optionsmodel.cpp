...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePlatinumAmount"))
        settings.setValue("nAnonymizePlatinumAmount", 1000);
    nAnonymizePlatinumAmount = settings.value("nAnonymizePlatinumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizePlatinumAmount"))
        SoftSetArg("-anonymizePlatinumamount", settings.value("nAnonymizePlatinumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePlatinumAmount:
            return QVariant(nAnonymizePlatinumAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizePlatinumAmount:
            nAnonymizePlatinumAmount = value.toInt();
            settings.setValue("nAnonymizePlatinumAmount", nAnonymizePlatinumAmount);
            emit AnonymizePlatinumAmountChanged(nAnonymizePlatinumAmount);
            break;
        default:
            break;
        }
